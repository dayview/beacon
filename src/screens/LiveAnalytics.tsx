...
export const LiveAnalytics: React.FC<LiveAnalyticsProps> = ({ onBack }) => {
    ...
    return (
      <div className="flex h-screen w-full items-center justify-center bg-[#fafafa]">
        <div className="text-center">
          ...
          <button
            onClick={onBack}
            className="inline-flex items-center gap-2 rounded-md bg-[#ffd02f] px-6 py-3 text-sm font-semibold text-[#050038] hover:bg-[#ffd02f]/90"
          >
            <ArrowLeft size={16} />
            Back to Dashboard
          </button>
  ...
  const analytics = selectedTest.analytics!;
  
  // Use real data from selected test
  const [stats, setStats] = useState({
    interactions: analytics.totalClicks,
    activeUsers: selectedTest.participants.current,
    avgTime: formatDuration(analytics.avgDuration),
    completion: `${analytics.completionRate}%`
  });

  // Simulate live stats update only if test is live
  useEffect(() => {
    if (selectedTest.status === 'live' || selectedTest.status === 'collecting') {
      const interval = setInterval(() => {
        setStats(prev => ({
          ...prev,
          interactions: prev.interactions + Math.floor(Math.random() * 3),
          activeUsers: selectedTest.participants.current + Math.floor(Math.random() * 3) - 1
        }));
      }, 2000);
      return () => clearInterval(interval);
    }
  }, [selectedTest.status, selectedTest.participants.current]);

  const handlePauseTest = () => {
    changeTestStatus(selectedTest.id, 'paused');
  };

  const handleStopTest = () => {
    changeTestStatus(selectedTest.id, 'completed');
  };

  const getStatusBadge = () => {
    const statusConfig: Record<typeof selectedTest.status, { label: string; variant: any }> = {
    ...
    const config = statusConfig[selectedTest.status];
                    ...
                    <div>
                        ...
                        <div className="bg-[#fafafa] p-4 rounded-lg space-y-2 text-sm">
                            <div className="flex justify-between">
                                ...
                                <span className="font-semibold text-[#050038]">{analytics.totalSessions}</span>
                            </div>
                            <div className="flex justify-between">
